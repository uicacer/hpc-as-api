...
| Metric | Value |
|--------|-------|
| Decode throughput (single user) | **~43 tok/s** |
| TTFT (warm, short prompt) | **~70ms** |
| TTFT (first ever request, Triton JIT) | ~3–5s (one-time spike) |
| Max concurrent 128K sessions | 6.47× |
| `torch.compile` — first run | ~68s |
| `torch.compile` — cached restarts | ~22s |

The Triton JIT spike only happens once — kernels are JIT-compiled on first use
and cached for all subsequent requests.

### Capacity for Prof. Chris's class (300 students)

At 43 tok/s single-user and ~10% duty cycle (students read/think between requests):
...
- **Synchronized "everyone run this now"**: queuing — last users may wait 2–5 min
